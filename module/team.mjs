...
const MasksTeamHUD = {
	...
	// --------- Encounter helpers (turn cards) ----------

	/**
	 * Collect the active combat (encounter) and all unique Character actors in it.
	 * We dedupe by actor id so a PC only gets one card even if they have multiple tokens.
	 */
	_collectEncounterActors() {
		const combat = game.combats?.active ?? null;
		if (!combat) return { combat: null, actors: [] };

		const map = new Map();
		const cbts = combat.combatants?.contents ?? combat.combatants ?? [];
		for (const cbt of cbts) {
			const actor = cbt?.actor;
			if (!actor || actor.type !== "character") continue;
			if (!map.has(actor.id)) map.set(actor.id, { actor, combatant: cbt });
		}

		const actors = [];
		for (const { actor, combatant } of map.values()) {
			const hpVal = Number(
				foundry.utils.getProperty(actor, "system.attributes.hp.value") ?? NaN
			);
			const isDowned = Number.isFinite(hpVal) && hpVal <= 0;
			actors.push({
				actor,
				actorId: actor.id,
				combatant,
	...
	/**
	 * Build the template context for the turn cards row.
	 * Reads per-combat cooldown state from flags[NS].turnTracker.
	 */
	_buildTurnCardContext() {
		const { combat, actors } = this._collectEncounterActors();
		...
		const rawState = combat.getFlag(NS, FLAG_TURN_TRACKER) ?? {};
		const storedActors =
			rawState &&
			typeof rawState === "object" &&
			rawState.actors &&
			typeof rawState.actors === "object"
				? rawState.actors
				: {};
		...
		for (const info of actors) {
			...
			const id = info.actorId;
			const isOwner = game.user?.isGM || actor.isOwner === true;

			// Potential = xp attribute (0–5 by default)
			const xpValRaw = Number(
				foundry.utils.getProperty(actor, "system.attributes.xp.value") ?? 0
			);
			const xpMaxRaw = Number(
				foundry.utils.getProperty(actor, "system.attributes.xp.max") ?? NaN
			);
			const xpMax = Number.isFinite(xpMaxRaw) && xpMaxRaw > 0 ? xpMaxRaw : 5;
			const potential = Math.min(
				xpMax,
				Math.max(0, Number.isFinite(xpValRaw) ? xpValRaw : 0)
			);
			const potentialRatio = xpMax > 0 ? potential / xpMax : 0;

			// Cooldown (per-actor, stored on the combat)
			const stored = storedActors[id] ?? {};
			const remainingRaw = Number(stored.remaining ?? 0);
			const maxRaw = Number(stored.max ?? 0);
			...
			cards.push({
				actorId: id,
				name,
				nameUpper: String(name).toUpperCase(),
		...
		return {
			hasEncounter: true,
			gmHasTurnCard: isGM && cards.length > 0,
			cards,
		};
	},

	/**
	 * Small helper to read/modify/write the per-combat turnTracker state.
	 * `mutator(state, encounterActors, combat)` may modify state. If it returns false,
	 * no write occurs.
	 */
	async _updateTurnState(mutator) {
		const { combat, actors: encounterActors } = this._collectEncounterActors();
		if (!combat || !encounterActors.length) {
			ui.notifications?.warn?.(
				"There is no active encounter with player characters."
			);
			return;
		}
		...
		const rawState = combat.getFlag(NS, FLAG_TURN_TRACKER) ?? {};
		const state = {
			actors: foundry.utils.deepClone(rawState.actors || {}),
		};

		const shouldWrite = mutator(state, encounterActors, combat);
		if (shouldWrite === false) return;

		try {
			await combat.setFlag(NS, FLAG_TURN_TRACKER, state);
		} catch (err) {
			console.error(`[${NS}] Failed to update turn tracker state`, err);
			ui.notifications?.error?.("Couldn’t update the turn tracker (see console).");
		}
	},

	/**
	 * GM-only: mark a specific character as having just taken their turn.
	 * - Their cooldown becomes (# other playable heroes at that moment).
	 * - All other heroes with a positive cooldown decrement by 1 (depleting right→left).
	 */
	async _markActorTurn(actorId) {
		if (!game.user?.isGM) return;

		await this._updateTurnState((state, encounterActors) => {
			const activeHeroes = encounterActors.filter((info) => !info.isDowned);
			if (!activeHeroes.length) return false;

			const target = activeHeroes.find((info) => info.actorId === actorId);
			if (!target) {
				ui.notifications?.warn?.("That character is not in the current encounter.");
				return false;
			}

			const teamSize = activeHeroes.length;
			const maxForTarget = Math.max(0, teamSize - 1);

			state.actors ??= {};

			// Normalize existing entries
			for (const info of activeHeroes) {
				const id = info.actorId;
				const prev = state.actors[id] || {};
				const remPrev = Number(prev.remaining ?? 0);
				const maxPrev = Number(prev.max ?? 0);
				state.actors[id] = {
					remaining: Math.max(0, Number.isFinite(remPrev) ? remPrev : 0),
					max: Math.max(0, Number.isFinite(maxPrev) ? maxPrev : 0),
				};
			}

			// Apply cooldown changes for this "step"
			for (const info of activeHeroes) {
				const id = info.actorId;
				if (!state.actors[id]) state.actors[id] = { remaining: 0, max: 0 };

				if (id === actorId) {
					state.actors[id].max = maxForTarget;
					state.actors[id].remaining = maxForTarget;
				} else if (state.actors[id].remaining > 0) {
					state.actors[id].remaining = Math.max(0, state.actors[id].remaining - 1);
				}
			}
		});
	},
	...
	/**
	 * GM-only: advance cooldowns as if a non-playable character (NPC, environment, etc.)
	 * has taken a turn. This just decrements any positive cooldown by 1.
	 */
	async _advanceCooldownForNpcTurn() {
		if (!game.user?.isGM) return;
		...
		await this._updateTurnState((state, encounterActors) => {
			...
			// Decrement everyone who’s currently cooling down
			for (const info of activeHeroes) {
				const id = info.actorId;
				if (state.actors[id].remaining > 0) {
					state.actors[id].remaining = Math.max(0, state.actors[id].remaining - 1);
				}
			}
		});
	...
	/**
	 * Increment a character’s Potential (xp) by 1 up to its max (default 5).
	 * Requires GM or actor owner.
	 */
	async _incrementPotential(actorId) {
		...
		const basePath = "system.attributes.xp";
		const valRaw = Number(
			foundry.utils.getProperty(actor, `${basePath}.value`) ?? 0
		);
		const maxRaw = Number(
			foundry.utils.getProperty(actor, `${basePath}.max`) ?? NaN
		);
		const max = Number.isFinite(maxRaw) && maxRaw > 0 ? maxRaw : 5;

		const current = Math.max(0, Number.isFinite(valRaw) ? valRaw : 0);
		if (current >= max) return;

		try {
			await actor.update({ [`${basePath}.value`]: current + 1 });
		} catch (err) {
			console.error(
				`[${NS}] Failed to increment Potential for ${actor.name}`,
				err
			);
	...
	async render() {
		...
		const { cards, hasEncounter, gmHasTurnCard } = this._buildTurnCardContext();
	...
	activateListeners() {
		...
		this.root
			?.querySelectorAll("[data-action='add-potential']")
			?.forEach((el) => {
				el.addEventListener("click", (ev) => {
					ev.preventDefault();
					ev.stopPropagation(); // don’t also trigger the card click
					const target = ev.currentTarget;
					const actorId = target?.dataset.actorId;
					if (!actorId) return;
					this._incrementPotential(actorId);
				});
			});
	...
	_registerHooks() {
		...
		// --- Turn cards: keep in sync with combat/actors/flags ---

		Hooks.on("updateCombat", (combat, changes) => {
			const flagsChanged =
				foundry.utils.getProperty(changes, `flags.${NS}.${FLAG_TURN_TRACKER}`) !==
				undefined;
			const activeChanged = Object.prototype.hasOwnProperty.call(
				changes,
				"active"
			);
			if (flagsChanged || activeChanged) this._queueRender();
		});
		...
		// Character updates that might affect cards: HP, xp/potential, name, image, etc.
