...
// Complete rewrite of Turn Cards system with simplified cooldown logic
...
(() => {
	...
	const FLAG_LAST_ACTION_TURN = "lastActionTurn";
	...
	function getTeamCombatants(combat) {
		return (combat?.combatants?.contents ?? []).filter((c) => c?.actor?.type === "character");
	}
	...
	async function setPotential(actor, val) {
		...
		try {
			...
			console.error(`[${NS}]`, e);
			ui.notifications?.warn?.("Cannot change Potential.");
		}
	}

	// ═══════════════════════════════════════════════════════════════════════════
	// Forward/Ongoing System
	// ═══════════════════════════════════════════════════════════════════════════
	...
	function statLabel(actor, key) {
		return getProp(actor, `system.stats.${key}.label`) ||
	...
	async function promptShiftLabels(actor, title) {
		...
		const makeOpts = (arr, check, limit, suffix) => labels.map((l) => {
			...
			const atLimit = check(l.value, limit);
	...
	async function applyShiftLabels(actor, upKey, downKey, { announce = true, reason = "shift", sourceActor = null } = {}) {
		...
		if (announce) {
			...
			let content = reason === "useInfluence" && sourceActor
		...
		async set(n, { announce = true, reason = null, actorName = null, delta = null } = {}) {
			...
			if (announce && game.settings.get(NS, "announceChanges")) {
				...
				if (reason === "aid" && actorName) {
					content = `<b>${escape(from)}</b> spends 1 Team to aid <b>${escape(actorName)}</b>!<br/>Team Pool: ${old} → <b>${n}</b><br/>${AID_MOVE_UUID}`;
				} else {
	...
	async function announceInfluenceChange(srcName, tgtName, beforeSym, afterSym) {
		...
		switch (afterSym) {
			...
			case "⬌": title = `${srcName} and ${tgtName} share Influence`; break;
			default: title = `${srcName} and ${tgtName} do not share Influence`;
		}
	...
	async function applyInfluencePair({ actorA, tokA, actorB, tokB, directive }) {
		...
		try {
			...
			console.error(`[${NS}]`, e);
			ui.notifications?.error?.("Couldn't update Influence.");
			return;
		}
	...
	// ═══════════════════════════════════════════════════════════════════════════
	// COOLDOWN SYSTEM - Complete Rewrite
	// ═══════════════════════════════════════════════════════════════════════════

	const FLAG_GLOBAL_TURN = "globalTurnCounter";

	const CooldownSystem = {
		getGlobalTurn(combat) {
			const val = combat?.getFlag?.(NS, FLAG_GLOBAL_TURN);
			return Number.isFinite(val) ? Math.max(0, Math.floor(val)) : 0;
		},

		getLastActionTurn(combatant) {
			const val = combatant?.getFlag?.(NS, FLAG_LAST_ACTION_TURN);
			return Number.isFinite(val) ? val : -Infinity;
		},

		getMaxCooldown(combat) {
			const team = getTeamCombatants(combat);
			return Math.max(0, team.length - 1);
		},

		isOnCooldown(combat, combatant) {
			const maxCooldown = this.getMaxCooldown(combat);
			if (maxCooldown <= 0) return false;
			const globalTurn = this.getGlobalTurn(combat);
			const lastAction = this.getLastActionTurn(combatant);
			return (globalTurn - lastAction) < maxCooldown;
		},

		getRemainingCooldown(combat, combatant) {
			const maxCooldown = this.getMaxCooldown(combat);
			if (maxCooldown <= 0) return 0;
			const globalTurn = this.getGlobalTurn(combat);
			const lastAction = this.getLastActionTurn(combatant);
			return Math.max(0, maxCooldown - (globalTurn - lastAction));
		},

		getCooldownFraction(combat, combatant) {
			const maxCooldown = this.getMaxCooldown(combat);
			if (maxCooldown <= 0) return 0;
			return this.getRemainingCooldown(combat, combatant) / maxCooldown;
		},

		async markAction(combat, combatant) {
			if (!combat || !combatant || !isGM()) return;
			const newGlobal = this.getGlobalTurn(combat) + 1;
			await combatant.setFlag(NS, FLAG_LAST_ACTION_TURN, newGlobal);
			await combat.setFlag(NS, FLAG_GLOBAL_TURN, newGlobal);
		},

		async advanceGlobal(combat) {
			if (!combat || !isGM()) return;
			await combat.setFlag(NS, FLAG_GLOBAL_TURN, this.getGlobalTurn(combat) + 1);
		},

		async resetAll(combat) {
			if (!combat || !isGM()) return;
			await combat.setFlag(NS, FLAG_GLOBAL_TURN, 0);
			for (const cbt of getTeamCombatants(combat)) {
				await cbt.unsetFlag(NS, FLAG_LAST_ACTION_TURN);
			}
		}
	};
	...
	const TurnCardsHUD = {
		...
		mount() {
			const host = document.querySelector("#ui-middle #ui-bottom") ||
		...
		async _handleAction(action, el, ev) {
			switch (action) {
				...
				case "forward":
					...
					break;
				case "team-action":
					await this._handleGmAction();
					break;
				case "card-action":
					await this._handleCardAction(el.dataset.combatantId);
					break;
				case "shift-labels":
				...
				case "team-reset":
					...
					else ui.notifications?.warn?.("Only GM can reset Team.");
					break;
			}
		},
		...
		async _handlePotential(el, delta) {
			...
			if (!actor || !canEdit(actor)) {
				ui.notifications?.warn?.("Can't change Potential.");
				return;
			}
			const cur = getPotential(actor);
			const next = clamp(cur + delta, 0, POTENTIAL_MAX);
			if (next !== cur) {
				await setPotential(actor, next);
		...
		async _handleForward(el, delta) {
			...
			const combat = getActiveCombat();
			const cbt = getTeamCombatants(combat).find((c) => c.actor?.id === targetActor.id);

			// Check if downed (cannot aid downed characters unless self or GM)
			if (cbt && isDowned(cbt) && !isSelf && !isGM()) {
				ui.notifications?.warn?.("Cannot aid downed character.");
				return;
			}
			...
			if (delta < 0) {
				if (!isSelf && !isGM()) {
					ui.notifications?.warn?.("Can only remove Forward from self.");
					return;
				}
				if (!canEdit(targetActor)) {
					ui.notifications?.warn?.("Can't edit that character.");
					return;
				}
			...
			// Adding forward
			if (isSelf || isGM()) {
				// Self or GM: free forward change, no team cost, no Aid UUID
				await this._applyForwardChange(targetActor, delta, false);
			} else {
				// Player adding forward to another character: costs 1 Team
				...
				if (hasActiveGM()) {
					...
					ui.notifications?.warn?.("GM must be online to aid.");
				}
			}
		},
		...
		async _applyForwardChange(actor, delta, isAid = false) {
			...
			try {
				...
				let content;
				if (isAid) {
					content = `<b>${name}</b> gains <b>+1 Forward</b> (now ${next}).<br/>${AID_MOVE_UUID}`;
				} else {
					content = `<b>${name}</b>: Forward ${cur} → <b>${next}</b> (${sign}${delta})`;
				}
				await ChatMessage.create({ content, type: CONST.CHAT_MESSAGE_TYPES.OTHER });
			} catch (e) {
				console.error(`[${NS}]`, e);
				ui.notifications?.error?.("Couldn't update Forward.");
			}
		},

		async _handleGmAction() {
			const combat = getActiveCombat();
			if (!combat) return;

			if (isGM()) {
				await CooldownSystem.advanceGlobal(combat);
				this._queueRender();
			} else {
				this._socketEmit({ action: "turnCardsGmTurn" });
			}
		},
		...
		async _handleCardAction(combatantId) {
			...
			if (isGM()) {
				await CooldownSystem.markAction(combat, cbt);
				this._queueRender();
			} else {
				this._socketEmit({ action: "turnCardsMark", combatantId });
			}
		},
		...
		async _handleShiftLabels(actorId) {
			...
			if (!canEdit(actor)) {
				ui.notifications?.warn?.("Can't shift Labels.");
				return;
			}

			const { canShiftUp, canShiftDown } = getShiftableLabels(actor);
			if (!canShiftUp.length || !canShiftDown.length) {
				ui.notifications?.warn?.("No valid shifts.");
				return;
			}
		...
		async _handleTeamChange(delta) {
			if (isGM() || TeamService.canEdit) {
				...
				this._socketEmit({ action: "turnCardsTeamChange", delta });
			}
		},

		_socketEmit(data) {
			try {
				game.socket?.emit(SOCKET_NS, data);
			} catch (e) {
				console.warn(`[${NS}] Socket emit failed`, e);
			}
		},
		...
		_registerHooks() {
			...
			Hooks.on("updateCombat", (doc, changes) => {
				...
				// Re-render on flag changes or active state changes
				const flagChanged = getProp(changes, `flags.${NS}`) !== undefined;
				if (flagChanged || isRelevant || changes?.active !== undefined) {
					this._queueRender();
				}
			});
			...
			Hooks.on("updateCombatant", (doc, changes) => {
				...
				// Re-render if defeated status or flags changed
				if (changes?.defeated !== undefined || getProp(changes, `flags.${NS}`) !== undefined) {
					this._queueRender();
				}
			});
			...
			Hooks.on("updateActor", (_actor, changes) => {
				...
				if (paths.some((p) => getProp(changes, p) !== undefined) ||
			...
			Hooks.on("updateJournalEntry", (doc) => {
				if (TeamService._doc && doc.id === TeamService._doc.id) {
					this._queueRender();
				}
			});
				...
				game.socket?.on(SOCKET_NS, async (data) => {
					...
					switch (data.action) {
						...
						case "turnCardsTeamChange": {
							if (data.delta) await TeamService.change(data.delta);
							break;
						}
						...
						case "turnCardsShiftLabels": {
							...
							if (reason === "useInfluence" && sourceActorId) {
								...
								if (!sourceActor || !InfluenceIndex.hasEdgeFromKeyToKey(
									compositeKey(sourceActor), compositeKey(target))) return;
							}
		...
		_setupContextMenu() {
			...
			if (this._contextMenu) {
				try { this._contextMenu.close?.(); } catch (_) { /* ignore */ }
		...
		async _resolveSource() {
			...
			if (my) {
				const tok = (canvas.tokens?.placeables ?? []).find((t) => t?.actor?.id === my.id) || null;
				return { token: tok, actor: my };
			}
		...
		async render() {
			...
			const maxCooldown = CooldownSystem.getMaxCooldown(combat);
			...
			const cards = team.map((cbt) => {
				...
				const ownsActor = canEdit(actor);
				const downed = isDowned(cbt);
				const isSelf = myActorId && actor.id === myActorId;

				const onCooldown = CooldownSystem.isOnCooldown(combat, cbt);
				const cooldownFrac = CooldownSystem.getCooldownFraction(combat, cbt);
				const remaining = CooldownSystem.getRemainingCooldown(combat, cbt);
				...
				// Hide Action button on busy (cooldown) or downed characters
				...
				return {
					...
					ariaLabel: [
						actor.name,
						downed ? "Downed" : null,
						onCooldown ? `Cooldown (${remaining})` : "Ready",
					...
					forwardDisabled,
					forwardTooltip: isSelf
						? `Forward: ${forward} | Ongoing: ${ongoing}`
						: canAid
							? `Aid ${actor.name}: Spend 1 Team`
							: "Cannot aid",
					status,
					statusTooltip: `Cooldown: ${remaining} turn(s)`,
					showStatusBar: onCooldown || downed,
					showActionBtn,
					actionAria: showActionBtn ? `Mark action for ${actor.name}` : "",
	...
	Hooks.once("ready", () => {
		try {
			...
			console.error(`[${NS}] Failed to mount TurnCardsHUD`, e);
		}
	});
})();
